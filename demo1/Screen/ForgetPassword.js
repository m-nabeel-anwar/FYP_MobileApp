...
var onsumbit=()=>
{
    ...
    if(data.Password.length===16 && data.Confrompassword.length===16 && format.test(data.Email))
    {
        ...
        //  var pas =md5(data.Password)
            ...
            axios.post('http://127.0.0.1:8000/ForgetPasswordCode',value)
            .then(req=>{
 ...
 var value=
  {
      ...
      Password:md5(data.Password),
          ...
          axios.post('http://127.0.0.1:8000/ForgetPassword',value)
                      .then(req=>{
                      if(req.data.Check==='True')
                      {
                        setdata({...data,Email:'',Password:'',Confrompassword:''})
...
<Animatable.View style={styles.footer} animation="fadeInUpBig">
  <ScrollView>
      <View style={{ width: '50%', height: 4, backgroundColor: '#D5DBDB',marginLeft:'25%',marginBottom:5 }} />
             ...
             <View>
             <Text style={styles.textstyle}>ConformPassword</Text>
            </View>
                    <View style={{flexDirection:'row'}}>
